...
public class SceneGraph extends JPanel {
	...
	private TransformedObject rotatingPoly; //tworzenie zmiennych obracających się wielokątów
	private TransformedObject rotatingPoly1;
	private TransformedObject rotatingPoly2;
	private TransformedObject rotatingPoly3;
	private TransformedObject rotatingPoly4;
	private TransformedObject rotatingPoly5;

	private TransformedObject Rect; //tworzenie zmiennych do prostokątów
	private TransformedObject Rect1;
	private TransformedObject Rect2;

	private TransformedObject Tri; //tworzenie zmiennych do trójkątów
	private TransformedObject Tri1;
	private TransformedObject Tri2;
	...
	private void createWorld() {
		...
		rotatingPoly  = new TransformedObject(filledPolygon); //utworzenie rotatingPoly, i zadeklarowanie że jest to wypełniony wielokąt
		rotatingPoly.setTranslation(-0.52,0.04).setScale(0.016, 0.016).setColor(Color.orange);
		//zdefinowanie że wielokąt ma zostać postawiony w settranslation x y, ustawiamy skalowanie x y, oraz kolor na pomarańczowy
		rotatingPoly1 = new TransformedObject(filledPolygon);
		rotatingPoly1.setTranslation(1.55,-0.75).setScale(0.016, 0.016).setColor(Color.orange);
		rotatingPoly2 = new TransformedObject(filledPolygon);
		rotatingPoly2.setTranslation(-3.22,1.67).setScale(0.012,0.012).setColor(Color.orange);
		rotatingPoly3 = new TransformedObject(filledPolygon);
		rotatingPoly3.setTranslation(-1.67,1.12).setScale(0.012,0.012).setColor(Color.orange);
		rotatingPoly4 = new TransformedObject(filledPolygon);
		rotatingPoly4.setTranslation(1.23,2.12).setScale(0.01,0.01).setColor(Color.orange);
		rotatingPoly5 = new TransformedObject(filledPolygon);
		rotatingPoly5.setTranslation(2.48,1.66).setScale(0.01,0.01).setColor(Color.orange);

		Rect  = new TransformedObject(filledRect); //utworzenie rect, zadeklarowanie, że jest wypełniony
		Rect.setTranslation(0.5,-0.35).setScale(2.3,0.15).setRotation(-20).setColor(Color.red);
		//ustawienie za pomocą settranslation x y, skala trójkata x y, obrót 20 stopni w lewo, ustawiamy kolor na czerwony
		Rect1 = new TransformedObject(filledRect);
		Rect1.setTranslation(-2.45,1.4).setScale(1.7,0.14).setRotation(-20).setColor(Color.red);
		Rect2 = new TransformedObject(filledRect);
		Rect2.setTranslation(1.85,1.9).setScale(1.4,0.12).setRotation(-20).setColor(Color.red);

		Tri  = new TransformedObject(filledTriangle);//utworzenie tri i zadeklarowanie że jest on wypełniony
		Tri.setScale(0.5,1.5).setTranslation(0.4,-1.8).setColor(Color.blue);
		//zdefinowanie skali x y, ustawienie punktu za pomocą settranslate x y, ustawienie koloru na niebieski
		Tri1 = new TransformedObject(filledTriangle);
		Tri1.setScale(0.4,1.3).setTranslation(-2.5,0.1).setColor(new Color(0x621865));
		Tri2 = new TransformedObject(filledTriangle);
		Tri2.setScale(0.3,1.2).setTranslation(1.85,0.7).setColor(new Color(0x1A7423));


		// TODO: Create objects and add them to the scene graph.
		world.add(rotatingPoly);
		world.add(rotatingPoly1);
		world.add(rotatingPoly2);
		world.add(rotatingPoly3);
		world.add(rotatingPoly4);
		world.add(rotatingPoly5);
	...
	public void updateFrame() {
		...
		rotatingPoly.setRotation(frameNumber*0.75);
		rotatingPoly1.setRotation(frameNumber*0.75);
		rotatingPoly2.setRotation(frameNumber*0.75);
		rotatingPoly3.setRotation(frameNumber*0.75);
		rotatingPoly4.setRotation(frameNumber*0.75);
		rotatingPoly5.setRotation(frameNumber*0.75);
